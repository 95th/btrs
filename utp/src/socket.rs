...
async fn take_address<A: ToSocketAddrs>(addr: A) -> Result<SocketAddr> {
    tokio::net::lookup_host(addr)
        .await?
        .next()
        .ok_or_else(|| SocketError::InvalidAddress.into())
}
...
impl UtpSocket {
    ...
    pub async fn bind<A: ToSocketAddrs>(addr: A) -> Result<UtpSocket> {
        ...
        let a = s.local_addr()?;
        Ok(UtpSocket::from_raw_parts(s, a))
    }
    ...
    pub async fn connect<A: ToSocketAddrs>(other: A) -> Result<UtpSocket> {
        ...
        for _ in 0..MAX_SYN_RETRIES {
            ...
            // Validate response
            let timeout = Duration::from_millis(syn_timeout);
            match tokio::time::timeout(timeout, socket.socket.recv_from(&mut buf)).await {
                Ok(result) => {
                    let (read, src) = result?;
...
///     let mut listener = UtpListener::bind(addr).await.expect("Error binding socket");
///
...
impl Incoming<'_> {
    pub async fn next(&mut self) -> Option<Result<(UtpSocket, SocketAddr)>> {
...
mod test {
    ...
    async fn test_sendto_on_closed_socket() {
        ...
        let mut buf = [0u8; BUF_SIZE];
        let (_read, _src) = server.recv_from(&mut buf).await.unwrap();
        assert_eq!(server.state, SocketState::Closed);
    ...
    async fn test_handle_packet() {
        ...
        let mut socket = UtpSocket::bind(server_addr).await.unwrap();
    ...
    async fn test_response_to_triple_ack() {
        ...
        for _ in 0..3u8 {
            server
                .socket
                .send_to(packet.as_ref(), server.connected_to)
                .await
                .unwrap();
        }
    ...
    async fn test_socket_timeout_request() {
        ...
        let client = UtpSocket::bind(client_addr).await.unwrap();
        let mut server = UtpSocket::bind(server_addr).await.unwrap();
        const LEN: usize = 512;
        let data = (0..LEN).map(|idx| idx as u8).collect::<Vec<u8>>();
        let d = data.clone();
        ...
        server.socket.recv_from(&mut buf).await.unwrap();
    ...
    async fn test_duplicate_packet_handling() {
        ...
        let child = tokio::spawn(async move {
            ...
            for _ in 0..2u8 {
                ...
                client
                    .socket
                    .send_to(packet.as_ref(), server_addr)
                    .await
                    .unwrap();
            }
            client.seq_nr += 1;
            ...
            for _ in 0..1u8 {
                let mut buf = [0; BUF_SIZE];
                client.socket.recv_from(&mut buf).await.unwrap();
            }
    ...
    async fn test_correct_packet_loss() {
        ...
        let child = tokio::spawn(async move {
            ...
            for (index, chunk) in chunks.enumerate() {
                ...
                if index % 2 == 0 {
                    client.socket.send_to(packet.as_ref(), dst).await.unwrap();
                }
        ...
        loop {
            match server.recv_from(&mut buf).await {
                Ok((0, _src)) => break,
                Ok((len, _src)) => received.extend(&buf[..len]),
                Err(e) => panic!("{}", e),
            }
        }
    ...
    async fn test_tolerance_to_small_buffers() {
        ...
        let child = tokio::spawn(async move {
            let mut client = UtpSocket::connect(server_addr).await.unwrap();
            client.send_to(&to_send[..]).await.unwrap();
            client.close().await.unwrap();
        });
        ...
        while server.state != SocketState::Closed {
            ...
            match server.recv_from(&mut small_buffer).await {
                Ok((0, _src)) => break,
                Ok((len, _src)) => read.extend(&small_buffer[..len]),
                Err(e) => panic!("{}", e),
            }
        }
    ...
    async fn test_sequence_number_rollover() {
        ...
        let child = tokio::spawn(async move {
            let mut client = UtpSocket::bind(client_addr).await.unwrap();
    ...
    #[tokio::test]
    async fn test_close_unused_socket() {
        let server_addr = next_test_ip4();
        let mut server = UtpSocket::bind(server_addr).await.unwrap();

        // Explicitly close socket. This test should not hang.
        server.close().await.unwrap();
    }
    ...
    async fn test_invalid_packet_on_connect() {
        ...
        let child = tokio::spawn(async move {
            ...
            match server.recv_from(&mut buf).await {
                Ok((_len, client_addr)) => {
                    server.send_to(&[], client_addr).await.unwrap();
                }
                _ => panic!(),
            }
        });
    ...
    async fn test_receive_unexpected_reply_type_on_connect() {
        ...
        let child = tokio::spawn(async move {
            ...
            match server.recv_from(&mut buf).await {
                Ok((_len, client_addr)) => {
                    server.send_to(packet.as_ref(), client_addr).await.unwrap();
                }
                _ => panic!(),
            }
        });
    ...
    async fn test_premature_fin() {
        ...
        server
            .socket
            .send_to(packet.as_ref(), client_addr)
            .await
            .unwrap();
