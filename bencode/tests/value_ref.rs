...
fn decode_none() {
    ...
    assert_eq!(e, Error::InvalidChar('a'));
}
...
fn decode_str_negative_len() {
    let e = ValueRef::decode(b"-10:helloworld").unwrap_err();
    assert_eq!(bencode::Error::InvalidChar('-'), e);
}
...
fn decode_expected_colon_dict() {
    ...
    assert_eq!(bencode::Error::ExpectedChar(':'), e);
}
